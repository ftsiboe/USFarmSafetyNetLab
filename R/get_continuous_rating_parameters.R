...
#' @export
get_continuous_rating_parameters <- function(year) {
  ...
  map_plan <- function(x) {
    # APH(90)->YP(1), CRC(44)->RP(2), RA(25)/IP(42)->RPHPE(3)
    data.table::fcase(
      x == 90, 1L,
      x == 44, 2L,
      x %in% c(25L, 42L), 3L,
  ...
  ADM[, insurance_plan_code := map_plan(insurance_plan_code)]
  ...
  Rd[, insurance_plan_code := map_plan(insurance_plan_code)]
