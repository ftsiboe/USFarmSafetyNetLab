...
SOB0 <- doBy::summaryBy(list(c("total_premium_amount","subsidy_amount"),c("commodity_year","insurance_plan_code","coverage_level_percent")),
                        data=as.data.frame(readRDS(paste0(dir_data_release,"/sob/sobtpu_all.rds"))),
                        FUN=sum,keep.names = T,na.rm=T)

ins_plan <- unique(as.data.frame(readRDS(paste0(dir_data_release,"/adm/fcip_recodes_insurance_plan.rds")))[
