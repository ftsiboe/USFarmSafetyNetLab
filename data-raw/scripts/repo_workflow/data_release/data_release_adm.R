...
# recodes practice
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_recodes_practice.R"))
...
# recodes palns
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_recodes_palns.R"))
...
# recodes commodity
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_recodes_commodity.R"))
...
# commodity price
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_commodity_price.R"))
...
# base rate
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_base_rate.R"))
...
# Contiguous county 
rm(list= ls()[!(ls() %in% c(Keep.List))])
contiguous_county <- clean_data(readRDS(paste0(farmpolicylab,"rmaFCIPdata/rmaActuarialDataMaster/Archive/2025/2025_A01230_ContiguousCounty_YTD.rds")))
...
rm(list= ls()[!(ls() %in% c(Keep.List))])
source(paste0("data-raw/scripts/repo_workflow/data_release/data_release_adm_premium_subsidy_schedule.R"))
