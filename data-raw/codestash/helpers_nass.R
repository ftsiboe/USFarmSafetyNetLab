...
process_nass_dataset <- function(
  ...
  df[grepl("CHICKPEAS",         commodity_name), commodity_name := "Dry Peas"]
...
#' head(df)
#' }
get_state_rental_rates <- function(dir_nass_qs = "./data-raw/fastscratch/nass_qs"){
...
#' Retrieve and process acreage data from USDA NASS Quick Stats
#'
#' This helper function reads the USDA National Agricultural
#' Statistics Service (NASS) Quick Stats “crops” dataset, filters it for
#' SURVEY/TOTAL/UNITED STATES/ANNUAL/YEAR, and the statistics “YIELD”,
#' “PRODUCTION”, “AREA PLANTED”, “AREA HARVESTED”, “AREA BEARING”. It then
#' computes irrigated vs. non‐irrigated vs. all practices acreage, resolves
#' overlaps, splits out crop classes, and finally returns three tables
#' (national, state, county) of planted, harvested, and bearing acres.
#'
#' @param dir_nass_qs Character. Path to the directory containing NASS Quick Stats raw files. Defaults to `"./data-raw/fastscratch/nass_qs"`.
#' @return A named list of three data.tables:
#'   \describe{
#'     \item{national}{Table of national‐level planted, harvested, bearing acres.}
#'     \item{state}{Table of state‐level planted, harvested, bearing acres.}
#'     \item{county}{Table of county‐level planted, harvested, bearing acres.}
#'   }
#' @import data.table
#' @importFrom doBy summaryBy
#' @importFrom tidyr gather
#' @export
get_acreage_data_nass <- function(
    dir_nass_qs = "./data-raw/fastscratch/nass_qs"){
  
  # 1. Read & preprocess the NASS “crops” dataset with desired filters
  df <- process_nass_dataset(
    dir_nass_qs = dir_nass_qs,
    large_dataset = "crops",
    nassqs_params = list( 
      ...
      freq_desc            = "ANNUAL", 
      reference_period_desc= "YEAR", 
      statisticcat_desc    = c("AREA PLANTED","AREA HARVESTED","AREA BEARING"), 
      agg_level_desc       = c("NATIONAL","STATE","COUNTY")
    )
  )

  # 2. Create separate columns for irrigated / non‐irrigated / all practices
  df[, `:=`(
    IRRIGATED     = fifelse(prodn_practice_desc == "IRRIGATED",     value, 0L),
    NON_IRRIGATED = fifelse(prodn_practice_desc == "NON-IRRIGATED", value, 0L),
    ALL_PRACTICES = fifelse(prodn_practice_desc == "ALL PRODUCTION PRACTICES", value, 0L)
  )]
  
  # 3. Sum those three practice‐types within each group
  df <- df[
    , lapply(.SD, sum), 
    by = c("statisticcat_desc","commodity_name","commodity_code","class_desc",
           "county_code","state_code","agg_level_desc","commodity_year"), 
    .SDcols = c("ALL_PRACTICES","IRRIGATED","NON_IRRIGATED")
  ]
  
  # 4. Resolve overlaps: 
  #    - If IRR + NON_IRR = ALL, drop ALL (set to zero)
  #    - If ALL ≠ 0, drop IRR & NON_IRR (set both to zero)
  df[IRRIGATED + NON_IRRIGATED == ALL_PRACTICES, ALL_PRACTICES := 0L]
  df[ALL_PRACTICES != 0, `:=`(IRRIGATED = 0L, NON_IRRIGATED = 0L)]
  
  # 5. Melt into long form: one row per practice‐type (irrg)
  df <- melt(
    df,
    id.vars       = c("statisticcat_desc","commodity_name","commodity_code",
                      "class_desc","county_code","state_code","agg_level_desc",
                      "commodity_year"),
    measure.vars  = c("ALL_PRACTICES","IRRIGATED","NON_IRRIGATED"),
    variable.name = "irrg",
    value.name    = "value"
  )[value != 0 & !is.na(value)]  # drop zero or NA values
  
  # 6. Handle crop classes: for each commodity,
  #    expand “ALL CLASSES” vs. individual class_desc,
  #    sum, resolve overlaps, then gather back to long
  df <- data.table::rbindlist(
    lapply(
      unique(df[["commodity_name"]]),
      function(CROP){
        data <- as.data.frame(df[commodity_name %in% CROP])
        classlist <- unique(data$class_desc)
        
        if("ALL CLASSES" %in% classlist & max(classlist)>1){
          data$classY_0 <- ifelse(data$class_desc %in% "ALL CLASSES",data$value,0)
          classlist <- classlist[! classlist %in% "ALL CLASSES"]
          for(xx in 1:length(classlist)){
            data[paste0("classX_",xx)] <- ifelse(data$class_desc %in% classlist[xx],data$value,0)
          }
          
          data <- doBy::summaryBy(list(c("classY_0",names(data)[grepl("classX_",names(data))]),
                                       c("statisticcat_desc","commodity_desc",
                                         "state_cd","county_cd","irrg","agg_level_desc","crop_yr")),
                                  data=data,FUN=sum,keep.names = T)
          
          data$classY_0 <- ifelse(rowSums(data[names(data)[grepl("classX_",names(data))]]) == data$classY_0,0,data$classY_0)
          
          for(xx in names(data)[grepl("classX_",names(data))]){
            data[,xx] <- ifelse(data$classY_0 != 0 ,0,data[,xx])
          }
          
          data <- data %>%  tidyr::gather(type, value, c("classY_0",names(data)[grepl("classX_",names(data))]))
          data <- data[!data$value %in% c(0,NA),]
          
          for(xx in 1:length(classlist)){
            data$type <- ifelse(data$type %in% paste0("classX_",xx),classlist[xx],data$type)
          }
          data$type <- ifelse(data$type %in% "classY_0","ALL CLASSES",data$type)
        }else{
          names(data)[names(data) %in% "class_desc"] <- "type"
        }
  ...
  setnames(df, "agg_level_desc", "agg_level")
  
  # 8. Recode statistic category to short desc and drop others
  df[, desc := fcase(
    statisticcat_desc == "AREA HARVESTED", "harvet",
    statisticcat_desc == "AREA PLANTED",   "plant",
    statisticcat_desc == "AREA BEARING",   "bearing",
    default = NA_character_
  )]
  df <- df[!is.na(desc)]  # keep only planted/harvested/bearing
  
  # 9. Append aggregation level to desc (e.g., "plant_STATE")
  df[, desc := paste0(desc, "_", agg_level)]
  
  # 10. Pivot wide by agg_level = STATE
  nass_survey_area_state <- dcast(
    df[agg_level == "STATE"],
    commodity_name + commodity_code + state_code + commodity_year ~ desc,
    value.var     = "value",
    fun.aggregate = sum
  )
  
  # 11. Pivot wide by agg_level = COUNTY
  nass_survey_area_county <- dcast(
    df[agg_level == "COUNTY"],
    commodity_name + commodity_code + state_code + county_code + commodity_year ~ desc,
    value.var     = "value",
    fun.aggregate = sum
  )
  
  # 12. Pivot wide by agg_level = NATIONAL
  nass_survey_area_national <- dcast(
    df[agg_level == "NATIONAL"],
    commodity_name + commodity_code + commodity_year ~ desc,
    value.var     = "value",
    fun.aggregate = sum
  )
  ...
  return(list(
    national = nass_survey_area_national,
    state = nass_survey_area_state,
    county   = nass_survey_area_county
  ))
