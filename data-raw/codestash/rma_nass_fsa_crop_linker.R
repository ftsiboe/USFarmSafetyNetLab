...
#----------------------------------------
# RMA CROPS                           ####
rma_crops <- readRDS(paste0(Dr.FCIP,"rmaActuarialDataMaster/Output/recodes/recodes_Crop_groupings.rds"))
...
#----------------------------------------
# NASS CROPS                          ####
nass_CENSUS <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/CENSUS/Area_Harvest/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/CENSUS/Area_Harvest/Area_Harvest_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_Harvest <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/SURVEY/Area_Harvest/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/SURVEY/Area_Harvest/Area_Harvest_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_Planted <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/SURVEY/Area_Planted/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/SURVEY/Area_Planted/Area_Planted_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_Bearing <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/SURVEY/Area_Bearing/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/SURVEY/Area_Bearing/Area_Bearing_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_Production <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/SURVEY/Production/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/SURVEY/Production/Production_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_Yield <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      as.numeric(gsub("[^0-9]","",list.files(paste0(Dr.NASS,"Output/SURVEY/Yield/")))),
      function(crop_yr){
        # crop_yr <- 2017
        area_nass <- readRDS(paste0(Dr.NASS,"Output/SURVEY/Yield/Yield_",crop_yr,".rds"))
        area_nass <- unique(area_nass[c("commodity_desc","year")])
        names(area_nass) <- c("crop_nass","crop_yr")
        return(area_nass)
      }), fill = TRUE))

nass_crops <- unique(rbind(nass_CENSUS,nass_Planted,nass_Harvest,nass_Bearing,nass_Yield,nass_Production))
rm(nass_CENSUS,nass_Planted,nass_Harvest,nass_Bearing,nass_Yield,nass_Production)
nass_crops$crop <- nass_crops$crop_nass
...
#----------------------------------------
# FSA CROPS                           ####
fsa_crops00 <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      list.files(paste0(Dr.FSA,"Archive/Output/"),full.names = T,pattern = ".rds") ,
      function(file){
        # file <- list.files(paste0(Dr.FSA,"Archive/Output/"),full.names = T)[1]
        area_fsa <- readRDS(file)
        area_fsa <- unique(area_fsa[c("crop_cd","crop","year")])
        names(area_fsa) <- c("crop_cd","crop_fsa","crop_yr")
        area_fsa$crop_cd_fsa <- area_fsa$crop_cd
        return(area_fsa)
      }), fill = TRUE))

fsa_crops <- as.data.frame(
  data.table::rbindlist(
    future_lapply(
      list.files(paste0(Dr.FSA,"Output/"),full.names = T) ,
      function(file){
        # file <- list.files(paste0(Dr.FSA,"Output/"),full.names = T)[1]
        area_fsa <- readRDS(file)
        area_fsa <- unique(area_fsa[c("crop_code","crop","crop_yr")])
        names(area_fsa) <- c("crop_cd","crop_fsa","crop_yr")
        area_fsa$crop_cd_fsa <- area_fsa$crop_cd
        return(area_fsa)
      }), fill = TRUE))

fsa_crops <- rbind(fsa_crops00[fsa_crops00$crop_yr< min(fsa_crops$crop_yr,na.rm=T),],fsa_crops)
fsa_crops <- unique(fsa_crops)
...
saveRDS(linker,file=paste0(Dr.FCIP,"rmaInsuredShare/Output/rma_nass_fsa_crop_linker.rds"))

#----------------------------------------


# crop_codes <- tempfile(fileext = ".rds")
# 
# download.file(
#   url = paste0("https://github.com/ftsiboe/USFarmSafetyNetLab/releases/download/adm/recodes_commodity_groupings.rds"),
#   destfile = crop_codes,
#   mode = "wb",
#   quiet = TRUE
# )
# 
# crop_codes <- readRDS(crop_codes)
# 
