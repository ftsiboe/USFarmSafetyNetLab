###############################################################################
# Project:   U.S. Federal Crop-Insurance Plan History
#
# Overview:
#   End-to-end pipeline that assembles a long-run (1989-present) panel of
#   Federal crop-insurance offerings, uptake, and actuarial performance.
#   The script
#     • loads the project workspace and custom plotting theme;
#     • downloads and cleans RMA Actuarial Data Master (ADM) and Summary-of-Business
#       (SOB) datasets;
#     • stitches historical cross-references (1995–2010) to current ADM metadata;
#     • reconciles plan abbreviations, constructs human-readable plan names, and
#       flags Basic vs. Endorsement provisions;
#     • aggregates SOB financials by crop year and plan, computes liability
#       shares (%) and cumulative loss ratios; and
#     • exports a tidy CSV plus a publication-quality timeline graphic
#       (“data-raw/figures/insurance_plan_history.*”).
#
# Why it matters:
#   A transparent, reproducible view of how crop-insurance products have evolved
#   helps researchers and policymakers evaluate policy mix, coverage gaps, and
#   program performance over time.
#
# Author:
#   Francis Tsiboe  ·  francis.tsiboe@xxxx.edu
#
# How to cite:
#   If this script (or the derived CSV/figure) informs your work, please ⭐️ the
#   repository and cite one or more of the following studies:
#     • Tsiboe, F. & Turner, D. (2025) “Incorporating buy-up price-loss coverage
#       into the United States farm safety net.” *Applied Economic Perspectives
#       and Policy*.
#     • Tsiboe, F. *et al.* (2025) “Risk-reduction impacts of crop insurance in
#       the United States.” *Applied Economic Perspectives and Policy*.
#     • Gaku, S. & Tsiboe, F. (2024) “Evaluation of alternative farm-safety-net
#       program combinations.” *Agricultural Finance Review*.
###############################################################################

rm(list = ls(all = TRUE)); gc()
devtools::document()

# Load custom workspace setup (defines Dr.FCIP, ers_theme(), etc.)
source("data-raw/codestash/work_station_setup.R")

cols <- c("#9DD9F7","#D7E5C8","#FEF389","#FFC425", "#BED73B", "#A0BD78","#00583D","#003524","#BE5E27")
...
# 7) Aggregate SOB financials by year & plan, join to recoded metadata
...
# Combine summed SOB and join recodes
...
# Replace missing names/policies with defaults
...
# 8) Summarize and normalize liability share by plan and year
...
# 9) Prepare data for plotting: assign ranks and filter
...
# 10) Define study plans and labels (for bolding in theme)
...
# Filter out unwanted categories and early years
...
# 11) Compute cumulative loss ratio (LR) for plotting
...
# Exclude those with <5 years span
...
# 12) Plot using ggplot2 and custom theme, then save outputs
library(ggplot2)
fig <- ggplot(data=data.fig, aes(x=crop_yr, y, fill = rank, color = rank,shape=rank, group=crop_yr)) +
  geom_point(size = 2.5) +
